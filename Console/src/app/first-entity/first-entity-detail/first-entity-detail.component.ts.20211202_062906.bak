...
import { FirstEntityService } from '@services'

import { FirstEntity } from '@models';
import { FirstEntityDetailData } from '../first-entity.component';

@Component({
  selector: 'app-first-entity-detail',
  templateUrl: './first-entity-detail.component.html',
  styleUrls: ['./first-entity-detail.component.scss']
})
export class FirstEntityDetailComponent implements OnInit {
  mode: string;
  model:FirstEntityDetail;
  
   firstEntityDetailForm: FormGroup;
...
column9Field: FormControl = new FormControl({ value: null, disabled: false }, [Validators.required] );
  ...
  constructor(
    public dialogRef: MatDialogRef<FirstEntityDetailComponent>,
    @Inject(MAT_DIALOG_DATA) public data: FirstEntityDetailData,
